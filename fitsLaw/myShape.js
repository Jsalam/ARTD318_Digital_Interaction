function myShape(p5, kind, posX, posY, width, seq){
	...
	this.makeGUI = function(){
		...
		this.sizeSlider = p5.createSlider(0, p5.height,50);
	...
	this.show = function(){
		...
		if (this.kind == 'rect'){
			...
			// show ellipse
			p5.ellipse(this.posX, this.posY, this.width);

		}

		if (this.editing){
			p5.stroke(0,120);
			p5.line(this.posX, this.posY - (this.width/2), this.posX, this.posY +(this.width/2));
			p5.line(this.posX - (this.width/2), this.posY, this.posX +(this.width/2), this.posY);
		} else{
			p5.stroke(0,50);
		}
		...
		if (!this.editing){
			if (index == this.seq){
				if (this.kind == 'rect'){
					...
					// evaluate proximity
					if (p5.dist(p5.mouseX, p5.mouseY, this.posX, this.posY) < this.width/2){
						if (p5.mouseIsPressed){
							this.wasClicked = true;
						}				
					}
